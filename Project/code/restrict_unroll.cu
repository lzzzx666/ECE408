// #define restrict_unroll
#ifdef restrict_unroll
#include <cmath>
#include <iostream>
...
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int B, const int M, const int C, const int H, const int W, const int K,const int S)
{
    ...
    */
    ...
    if(w < W_out && h < H_out)
    {
            ...
            for (int p = 0; p < K; p++)
            {
                ...
                for(; q <= K - 2; q += 2)
                {
                    if(h * S + p < H && w * S + q < W)
                    {
                        ...
                        acc += in_4d(n, c, h * S + p, w * S + q + 1) * mask_4d(m, c, p, q + 1);
        ...
        // int q = 0;
        //         for(; q <= K - 2; q += 2) // Unrolling by 2
        //         {
        //             acc += share_2d(y * S + p, x * S + q) * mask_4d(m, c, p, q);
        //             acc += share_2d(y * S + p, x * S + q + 1) * mask_4d(m, c, p, q + 1);
        //         }
        //         // Handle the remainder if K is odd
        //         if (q < K) {
        //             acc += share_2d(y * S + p, x * S + q) * mask_4d(m, c, p, q);
        //         }
        out_4d(n, m, h, w) = acc;
...
const int B, const int M, const int C, const int H, const int W, const int K, const int S)
{
    ...
    cudaMemset(*device_output_ptr, 0, B * M * H_out * W_out * sizeof(float));
