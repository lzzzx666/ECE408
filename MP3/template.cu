...
__global__ void matrixMultiplyShared(float *A, float *B, float *C,
  ...
  float Pvalue = 0;
  for(int q = 0; q < (numAColumns + TileWidth - 1) / TileWidth; q++)
  {
    if(q * TileWidth + tx >= numAColumns) 
    {
      subTileM[ty][tx] = 0;
    }
    else 
...
int main(int argc, char **argv) {
  ...
  dim3 Dimgrid(ceil(double(numCColumns)/ 16.0), ceil(double(numCRows)/ 16.0), 1);
